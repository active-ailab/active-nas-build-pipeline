# Release FW Pipeline 使用说明 (飞书 CLI 版 v2.0)

> **v2.0 重构**: 飞书交互已迁移至 Lark CLI，代码量减少 60%+，认证一步完成。详见 `lark_cli_adapter.py`

本仓库的主入口脚本是 `lark_release.py`（统一 CLI），用于在 Windows（PowerShell）环境下串联执行：

1. 从 Jenkins 下载产物（Debug / Release）
2. 运行可配置的 Prepare 流程（解压、挑选文件、拷贝到 payload 等）
3. 通过 NAS WebDAV 上传
4. 通过 Synology DSM FileStation 生成共享链接
5. 生成本地版本文档（Markdown）
6. **通过飞书 CLI 自动创建云端文档 / 知识库页面 / 多维表格记录**
